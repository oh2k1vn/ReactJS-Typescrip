  ...
  const handleDropdownValue = (e: any) => {
    if (e.value == "Khám tim mạch") {
      const filterdata = data.filter(
        (item: any) => item.service == "Khám tim mạch"
      );
      ...
      const filterdata = data.filter(
        (item: any) => item.service == "Khám tổng quát"
      );
      setDataReport(filterdata);
    } else if (e.value == "Đang chờ") {
      const filterdata = data.filter(
        (item: any) => item.stateFilter == "Đang chờ"
      );
      setDataReport(filterdata);
    } else if (e.value == "Đã sử dụng") {
      const filterdata = data.filter(
        (item: any) => item.stateFilter == "Đã sử dụng"
      );
      setDataReport(filterdata);
    } else if (e.value == "Bỏ qua") {
      const filterdata = data.filter(
        (item: any) => item.stateFilter == "Bỏ qua"
      );
      setDataReport(filterdata);
    ...
    <div className="report">
      <div className="reportinfo">
        <div className="reportinfolist">
          <p className="reporttitle">Báo cáo</p>

          <img src={reportgoto} alt="reportgoto" className="reportgoto"></img>

          <p className="reportlist ">Lập báo cáo</p>
        </div>
        <div className="bell">
          <Bellactive />
        </div>
      ...
      <div className="reportcontainer">
        <div className="reportlisttitle">Chọn thời gian</div>
            ...
            <div className="listreport">
              ...
              {dataReport.map((item: any, index: any) => (
                <div className="reporttablelist" key={index}>
                  <div className="reportitemnameid" style={{ width: "100%" }}>
                    <p className="reportitemnametitleid">{item.id}</p>
                  </div>
                  <div className="reportitemnameid" style={{ width: "100%" }}>
                    <p className="reportitemnametitleid">{item.name}</p>
                  </div>
                  <div className="reportitemnameid" style={{ width: "100%" }}>
                    <p className="reportitemnametitleid">{item.time}</p>
                  </div>
                  <div className="reportitemnameid" style={{ width: "100%" }}>
                  ...
                  <div className="reportitemnameid" style={{ width: "100%" }}>
                    <p className="reportitemnametitleid">{item.sourse}</p>
                  </div>
                </div>
              ))}
            </div>
