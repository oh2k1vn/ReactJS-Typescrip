  ...
  const handleDropdownValue = (e: any) => {
    if (e.value == "Đang chờ") {
      const filterdata = dataNumberLevel.filter(
        (item: any) => item.status === "Đang chờ"
      );
      setData(filterdata);
    } else if (e.value == "Đã sử dụng") {
      const filterdata = dataNumberLevel.filter(
        (item: any) => item.status === "Đã sử dụng"
      );
      setData(filterdata);
    } else if (e.value == "Bỏ qua") {
      const filterdata = dataNumberLevel.filter(
        (item: any) => item.status === "Bỏ qua"
      );
      setData(filterdata);
    } else if (e.value == "Khám răng hàm mặt") {
      const filterdata = dataNumberLevel.filter(
        (item: any) => item.service === "Khám răng hàm mặt"
      );
      setData(filterdata);
    } else if (e.value == "Khám tai mũi họng") {
      const filterdata = dataNumberLevel.filter(
        (item: any) => item.service === "Khám tai mũi họng"
      );
      setData(filterdata);
    } else if (e.value == "Khám sản - Phụ khoa") {
      const filterdata = dataNumberLevel.filter(
        (item: any) => item.service === "Khám sản - Phụ khoa"
      );
      setData(filterdata);
    } else if (e.value == "Kiosk") {
      const filterdata = dataNumberLevel.filter(
        (item: any) => item.sourse === "Kiosk"
      );
      setData(filterdata);
    } else if (e.value == "Hệ thống") {
      const filterdata = dataNumberLevel.filter(
        (item: any) => item.sourse === "Hệ thống"
      );
      setData(filterdata);
    } else {
    ...
    <div className={classes.level}>
      <div className={classes.levelinfo}>
        <div className={classes.levelinfolist}>
          <p className={classes.leveltitle}>Cấp số</p>

          <img
            src={Levelgoto}
            alt="levelgoto"
            className={classes.levelgoto}
          ></img>

          <p className={classes.levellist}>Danh sách cấp số</p>
        </div>
      ...
      <div className={classes.levelcontainer}>
        <div className={classes.levellisttitle}>Quản lý cấp số</div>
        ...
        <div className={classes.tablelistitem}>
          ...
          {data.map((item: any, index: any) => (
            <div className={classes.leveltablelist} key={index}>
              ...
              <div
                className={classes.levelitemlistnew}
                onClick={() => handleSend(item)}
              >
                {" "}
                <Link to="/level/service">Chi tiết</Link>{" "}
              </div>
            </div>
          ))}
        </div>
      </div>
      <Link to="/level/add-level">
        {" "}
